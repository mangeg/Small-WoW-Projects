...
function Debug:AddGlobal(name, data)
	...
	_G[("LibWPF_%s"):format(name)] = data
end

function Debug:Print(level, ...)
	expect(level, "typeof", "number")
	if level > self.db.profile.DebugLevel then return end
	local t = ...
	if select(2, ...) then
		t = {...}
	end
	print(("|cffff7e00RW |r(|cffff7e00%d|r)|cffff7e00:|r %s"):format(level, pretty_tostring(t)))
end
